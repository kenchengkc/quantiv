#!/usr/bin/env python3
"""
Simplified expected move labels and features builder.

Creates em_labels, em_features, and em_training tables/views in DuckDB
bypassing timezone conversion issues by using pandas for data processing.

Usage:
  python scripts/build_em_simple.py --local
"""
...
def create_simple_labels(conn):
    """Create simplified labels using IV proxy."""
    ...
    try:
        ...
        # Get volatility data with explicit date casting
...
def create_simple_features(conn):
    """Create simplified features table."""
    ...
    try:
        ...
        vol_features = pd.read_sql_query("""
            SELECT act_symbol,
                   CAST(trade_date AS DATE) as trade_date,
                   iv_current,
                   hv_current,
        ...
        # Get options features (simplified)
        opt_features = pd.read_sql_query("""
            SELECT act_symbol,
                   CAST(trade_date AS DATE) as trade_date,
            ...
            GROUP BY act_symbol, trade_date
        """, conn)
        ...
        if features_list:
            ...
            conn.execute("CREATE TABLE em_features AS SELECT * FROM features_df")
...
def main():
    parser = argparse.ArgumentParser(description="Build EM labels and features (simplified)")
    parser.add_argument("--local", action="store_true", help="Use local data directory")
    parser.add_argument("--duckdb-path", type=Path, help="DuckDB database path")
    ...
    if args.local:
        ...
        duckdb_path = args.duckdb_path or Path("/srv/quantiv-data/quantiv.duckdb")
    
    print("[setup] Building EM labels and features (simplified)")
    ...
    if create_simple_labels(conn):
        success_count += 1
    
    # Step 2: Create features
    if create_simple_features(conn):
        success_count += 1
    ...
    print(f"\n[success] Simplified EM pipeline completed ({success_count}/{total_steps} steps)")
