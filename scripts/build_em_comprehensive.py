#!/usr/bin/env python3
"""
Comprehensive expected move pipeline with expanded synthetic data.

Creates enough training samples for model training.

Usage:
  python scripts/build_em_comprehensive.py --local
"""
...
def load_all_data(data_dir):
    ...
    # Load options data (sample for performance)
    opt_files = glob.glob(str(data_dir / "parquet" / "options_chain" / "**" / "*.parquet"), recursive=True)
    opt_dfs = []
    
    # Sample files for better performance
    sample_files = random.sample(opt_files, min(50, len(opt_files)))
...
def create_comprehensive_earnings(vol_symbols, vol_date_range, target_samples=100):
    """Create comprehensive synthetic earnings dates."""
    print(f"[earnings] Creating {target_samples} synthetic earnings dates...")
    
    if not vol_symbols or not vol_date_range[0]:
        print("[earnings] ⚠ No volatility data available")
        return pd.DataFrame()
    
    # Get common symbols that likely have good data coverage
    common_symbols = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'NFLX', 
                     'CRM', 'ORCL', 'ADBE', 'PYPL', 'UBER', 'LYFT', 'SNAP', 'AMD', 
                     'INTC', 'IBM', 'BA', 'GE', 'JPM', 'BAC', 'WFC', 'C', 'GS',
                     'JNJ', 'PFE', 'MRK', 'UNH', 'CVS', 'WMT', 'TGT', 'HD', 'LOW',
                     'DIS', 'CMCSA', 'VZ', 'T', 'COST', 'SBUX', 'MCD', 'KO', 'PEP']
    
    available_symbols = [s for s in common_symbols if s in vol_symbols]
    
    if len(available_symbols) < 10:
        # Use any available symbols
        available_symbols = list(vol_symbols)[:50]
    
    print(f"[earnings] Using {len(available_symbols)} symbols")
    
    # Create earnings dates across the data range
    start_date = vol_date_range[0] + timedelta(days=10)
    end_date = vol_date_range[1] - timedelta(days=10)
    
    earnings_list = []
    
    # Generate quarterly earnings for each symbol
    for symbol in available_symbols:
        current_date = start_date
        
        while current_date <= end_date and len(earnings_list) < target_samples:
            # Add some randomness to earnings dates
            earnings_date = current_date + timedelta(days=random.randint(0, 30))
    ...
    random.shuffle(earnings_list)
    earnings_list = earnings_list[:target_samples]
...
def create_labels_and_features(vol_data, opt_data, earnings_df):
    ...
    if not opt_data.empty:
        ...
        print("[pipeline] Aggregating options data...")
        ...
        call_counts = opt_data[opt_data['call_put'] == 'C'].groupby(['act_symbol', 'trade_date']).size()
        put_counts = opt_data[opt_data['call_put'] == 'P'].groupby(['act_symbol', 'trade_date']).size()
        ...
        print(f"[pipeline] Aggregated options data: {len(opt_agg)} symbol-date combinations")
    else:
    ...
    print("[pipeline] Processing earnings events...")
    for i, (_, earning) in enumerate(earnings_df.iterrows()):
        if i % 20 == 0:
            print(f"[pipeline] Processing {i+1}/{len(earnings_df)} earnings events...")
        ...
        # Post-earnings IV (1-7 days after)
        post_start = earnings_date + timedelta(days=1)
        post_end = earnings_date + timedelta(days=7)
        post_vol = symbol_vol[
        ...
        if len(pre_vol) >= 3 and len(post_vol) >= 2:
            ...
            if iv_pre > 0:
                ...
                feature_row.update({
                    ...
                    'iv_percentile_est': min(max((latest_vol['iv_current'] - latest_vol.get('iv_year_low', 0)) / 
                                                max(latest_vol.get('iv_year_high', 1) - latest_vol.get('iv_year_low', 0), 0.01), 0), 1)
                                        if pd.notna(latest_vol.get('iv_year_high')) and pd.notna(latest_vol.get('iv_year_low')) else 0.5
                })
                ...
                if not opt_agg.empty:
                    ...
                    pre_opt = symbol_opt[
                        ...
                        (symbol_opt['trade_date'] >= earnings_date - timedelta(days=7))
                    ...
                    if len(pre_opt) > 0:
                        ...
                        feature_row.update({
                            ...
                            'iv_skew': random.uniform(-0.05, 0.05),  # Synthetic skew
                            'avg_gamma_t1': latest_opt['avg_gamma_t1'],
                            'avg_vega_t1': latest_opt['avg_vega_t1'],
                            'call_put_ratio': latest_opt.get('call_put_ratio', 1.0),
                            'total_contracts': latest_opt['total_contracts']
                        })
                    else:
                        # Add default options features
                        feature_row.update({
                            'avg_iv_t1': latest_vol['iv_current'],
                            'atm_iv_t1': latest_vol['iv_current'],
                            'iv_skew': 0.0,
                            'avg_gamma_t1': 0.05,
                            'avg_vega_t1': 0.1,
                            'call_put_ratio': 1.0,
    ...
    try:
        ...
        if not features_df.empty:
            ...
            if training_count > 0:
                ...
                print(f"  Target variable (y) stats: min={stats[0]:.3f}, max={stats[1]:.3f}, avg={stats[2]:.3f}")
...
def main():
    parser = argparse.ArgumentParser(description="Comprehensive EM pipeline")
    parser.add_argument("--local", action="store_true", help="Use local data directory")
    parser.add_argument("--samples", type=int, default=100, help="Target number of training samples")
    ...
    print("[setup] Comprehensive EM pipeline")
    print(f"[setup] Data dir: {data_dir}")
    print(f"[setup] Target samples: {args.samples}")
    ...
    earnings_df = create_comprehensive_earnings(vol_symbols, vol_date_range, args.samples)
    ...
    if duckdb_path.exists():
        ...
        if success and not labels_df.empty and not features_df.empty:
            print(f"\n✓ Comprehensive EM pipeline completed successfully!")
