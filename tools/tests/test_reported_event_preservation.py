...
def test_archived_ml_wins_over_historical_iv_and_iv_is_preserved():
    detail = {
        "expected_move": None,
        "earnings_history": [
            {
    ...
    assert result["expected_move"]["display_forecast_method"] == "ml"
    assert result["expected_move"]["display_forecast_pct"] == pytest.approx(0.036898)
    assert result["expected_move"]["em_ml_pct"] == 0.036898
    assert result["expected_move"]["ml_status"] == "available"



def test_reported_calendar_and_symbol_share_the_same_frozen_ml_forecast(
    ...
    assert changed == 1
    assert calendar[0]["display_forecast_method"] == "ml"
    assert calendar[0]["display_forecast_pct"] == pytest.approx(0.055115)
    ...
    assert symbol["expected_move"]["display_forecast_method"] == "ml"
    assert symbol["expected_move"]["display_forecast_pct"] == pytest.approx(0.055115)
