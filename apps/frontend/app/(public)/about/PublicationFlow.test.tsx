...
it('walks through a valid release and stops on the published result', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Consistent publication' })).toBeTruthy();
...
it('allows inspection without auto-advancing and respects reduced motion', () => {
  ...
  expect(screen.getByRole('heading', { name: 'Point-in-time inputs' })).toBeTruthy();
  fireEvent.click(screen.getByRole('button', { name: /03 Verify/ }));
  expect(screen.getByRole('heading', { name: 'Forecast consistency' })).toBeTruthy();
