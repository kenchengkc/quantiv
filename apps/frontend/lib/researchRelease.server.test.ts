...
describe('retained Research Lab release verification', () => {
  ...
  it('parses the exact source bytes covered by the verified binding', () => {
    const { root, deployment } = fixture();
    const verified = readVerifiedRetainedPublicJson<{ schema: string }>(
      'research-history.json',
      root,
      deployment,
    );

    expect(verified.binding.release_id).toBe(deployment.release_id);
    expect(verified.value).toEqual({ schema: 'quantiv.historical-event-universe.v1' });
  });
