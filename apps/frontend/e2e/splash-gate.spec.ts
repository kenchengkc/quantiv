...
test.describe('homepage splash', () => {
  test('first visit paints the server-rendered intro without hiding the app shell', async ({ page }) => {
    await page.addInitScript((key) => sessionStorage.removeItem(key), SPLASH_SESSION_KEY);
    ...
    await expect(page.locator('head > script#quantiv-splash-session')).toHaveCount(1);
    await expect(page.locator('.quantiv-splash')).toBeVisible();
    await expect(page.locator('.quantiv-splash-wordmark')).toHaveText('QUANTIV');
  ...
  test('returning session skips the intro before paint', async ({ page }) => {
    ...
    const skip = await page.evaluate((attribute) =>
