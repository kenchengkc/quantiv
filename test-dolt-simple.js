...
/**
 * Simple test script to verify Dolt integration with direct API calls
 */

async function testDoltIntegration() {
  console.log('🧪 Testing Dolt Integration with Direct API Calls\n');

  // Use the endpoints from your .env.local or defaults
  const chainEndpoint = process.env.DOLT_CHAIN_ENDPOINT || 'https://www.dolthub.com/api/v1alpha1/post-no-preference/options/master';
  const ivEndpoint = process.env.DOLT_IV_ENDPOINT || 'https://www.dolthub.com/api/v1alpha1/post-no-preference/options/master';

  console.log('Chain Endpoint:', chainEndpoint);
  console.log('IV Endpoint:', ivEndpoint);

  // Test 1: Basic connection and symbol availability
  console.log('\n📋 Test 1: Check available symbols');
  try {
    const sql = 'SELECT DISTINCT act_symbol FROM option_chain LIMIT 10';
    const url = `${chainEndpoint}?q=${encodeURIComponent(sql)}`;
  ...
  console.log('\n📊 Test 2: Check BLK data availability');
  try {
    const sql = "SELECT COUNT(*) as count FROM option_chain WHERE act_symbol = 'BLK'";
    const url = `${chainEndpoint}?q=${encodeURIComponent(sql)}`;
  ...
  try {
    ...
    const url = `${ivEndpoint}?q=${encodeURIComponent(sql)}`;
