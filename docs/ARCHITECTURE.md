...
The earnings calendar retains a provisional `CLOSE` move across midnight when a cached Finnhub quote belongs to a completed regular session and its `quoteCloseDate` matches the event's resolving session (BMO: report day; AMC: next trading day). This is the last available cached daily move, including trades before the closing bell and the existing quote-settle window, rather than finalized OHLCV. Pre-open warmup quotes are excluded. The per-symbol date never comes from the batch timestamp; replacement quotes clear it when they lack closing-session provenance. Event-matched `REALIZED` data takes precedence when the nightly publication or Polygon backfill arrives. This fallback only reads existing cache entries and makes no provider calls.
