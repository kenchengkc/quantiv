...
3. the content-addressed `data/models/receipts/latest_models.json` receipt and its `models` scope / passed quality;
4. equality between the signed manifest's `receipt_id` and the recomputed model-validation receipt ID;
...
The focused provenance tests cover receipt tampering, invalid signed pointers, altered signed-bundle metadata, model-validation receipt mismatch, forecast/model mismatch, and run-derived protocol projection. The normal repository CI also runs the broader frontend, Python, generated-data, worker, and container checks.
